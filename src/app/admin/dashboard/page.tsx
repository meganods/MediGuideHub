...
function AdminContent() {
  ...
  const handleExportMessagesPDF = () => {
    ...
    const messagesHtml = messages.map(m => `
      <tr>
        ...
        <td>${m.subject}</td>
        <td>${m.inquiryType || "General Question"}</td>
        <td>${m.message}</td>
        <td>${m.createdAt}</td>
        <td>
          <span class="status-badge ${m.resolved ? 'resolved' : 'pending'}">
    ...
    const htmlContent = `
      <html>
        <head>
          <title>mediguide4u - Visitor Inquiries Report</title>
          <style>
            ...
            th, td { padding: 12px; border: 1px solid #f0eada; font-size: 13px; text-align: left; }
            th { font-weight: bold; background-color: #fdfbf7; color: #113f48; border-bottom: 2px solid #C9A15A; }
            .status-badge { display: inline-block; padding: 2px 8px; border-radius: 9999px; font-size: 10px; font-weight: bold; }
            .status-badge.resolved { background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
            .status-badge.pending { background-color: #fef2f2; color: #b91c1c; border: 1px solid #fecaca; }
            .footer { margin-top: 50px; font-size: 11px; color: #888; text-align: center; border-top: 1px solid #eee; padding-top: 20px; }
          </style>
        </head>
        <body>
          <h1>mediguide4u — Visitor Inquiries Inventory Report</h1>
          <table>
            <thead>
              <tr>
          ...
          <div class="footer">
            Document generated by mediguide4u Admin Console on ${new Date().toLocaleDateString()} at ${new Date().toLocaleTimeString()}.
          </div>
        </body>
      </html>
    `;
  ...
  useEffect(() => {
    ...
    if (isFirebaseConfigured && adminFirestore) {
      ...
      const interval = setInterval(() => {
        const localMsgs = JSON.parse(localStorage.getItem("mediguide4u_contacts") || "[]") as ContactMessage[];
  ...
  const handleExportSubscribersPDF = () => {
    ...
    const htmlContent = `
      <html>
        <head>
          <title>mediguide4u - Newsletter Subscribers Report</title>
          <style>
            body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #113f48; padding: 40px; line-height: 1.6; }
            h1 { color: #113f48; border-bottom: 2px solid #C9A15A; padding-bottom: 10px; margin-bottom: 30px; font-size: 28px; }
            table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
        ...
        <body>
          <h1>mediguide4u — Newsletter Subscribers Inventory Report</h1>
          <table>
            <thead>
              <tr>
    ...
    <div className="min-h-screen bg-[#F9FAFB] overflow-x-hidden">
        ...
        <div className="w-full mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="font-bold text-sm sm:text-base md:text-lg text-[#113F48] tracking-tight flex items-center gap-1.5 min-w-0 truncate">
              <span className="hidden sm:inline truncate">mediguide4u</span>
              <span className="sm:hidden">mediguide4u</span>
      ...
      <div className="w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-[#113F48] p-4 shadow-2xl transform transition-transform duration-300 ease-out lg:static lg:translate-x-0 lg:w-auto lg:col-span-3 lg:bg-transparent lg:p-0 lg:shadow-none lg:sticky lg:top-28 ${isSidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
            <div className="flex items-center justify-between lg:hidden mb-6">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-white">mediguide4u</span>
              </div>
                        ...
                        <div className="space-y-2 border border-dashed border-[#C9A15A]/30 p-4 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#FDF6EC]/10">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-stone-400">Upload to show on the website.</p>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                          ...
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              placeholder="Source: mediguide4u Research"
                              className="w-full bg-[#FDF6EC]/10 border border-stone-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-1 focus:ring-[#C9A15A] text-[#113F48]"
                            />
                          </div>
                        </div>
                        ...
                        onClick={() => {
                          const articlesHtml = posts.map(p => `
                            <tr>
                              ...
                              <td>${p.category}</td>
                              <td><span class="status-badge ${p.status === 'Draft' ? 'draft' : 'published'}">${p.status || "Published"}</span></td>
                              <td>${p.seoScore || 85}/100</td>
                              <td>${p.views || 0}</td>
                              <td>${p.publishedAt}</td>
                            </tr>
                          `).join("");
                          ...
                          const htmlContent = `
                            <html>
                              <head>
                                <title>mediguide4u - Blog Articles Export</title>
                                <style>
                                  body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #113f48; padding: 40px; line-height: 1.6; }
                                  h1 { color: #113f48; border-bottom: 2px solid #C9A15A; padding-bottom: 10px; margin-bottom: 30px; font-size: 28px; }
                                  table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
                                  ...
                                  th { font-weight: bold; background-color: #fdfbf7; color: #113f48; border-bottom: 2px solid #C9A15A; }
                                  .status-badge { display: inline-block; padding: 2px 8px; border-radius: 9999px; font-size: 10px; font-weight: bold; }
                                  .status-badge.published { background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
                                  .status-badge.draft { background-color: #f3f4f6; color: #4b5563; border: 1px solid #e5e7eb; }
                                  .footer { margin-top: 50px; font-size: 11px; color: #888; text-align: center; border-top: 1px solid #eee; padding-top: 20px; }
                                </style>
                              </head>
                              <body>
                                <h1>mediguide4u — Articles Inventory Report</h1>
                                <table>
                                  <thead>
                                    <tr>
                                      <th>Title</th>
                      ...
                      <button
                        onClick={() => {
                          const categoriesHtml = categoriesList.map(c => {
                            const articleCount = posts.filter(p => p.category === c.name).length;
                            return `
                              <tr>
                                <td><strong>${c.name}</strong></td>
                                <td><code>${c.slug}</code></td>
                                <td>${c.displayOrder || 0}</td>
                                <td>${c.featuredCategory ? "Yes" : "No"}</td>
                                <td><span class="status-badge ${c.status === 'Inactive' ? 'inactive' : 'active'}">${c.status || "Active"}</span></td>
                                <td>${articleCount} Guides</td>
                              </tr>
                            `;
                          }).join("");
                          ...
                          const htmlContent = `
                            <html>
                              <head>
                                <title>mediguide4u - Categories Inventory Export</title>
                                <style>
                                  body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #113f48; padding: 40px; line-height: 1.6; }
                                  h1 { color: #113f48; border-bottom: 2px solid #C9A15A; padding-bottom: 10px; margin-bottom: 30px; font-size: 28px; }
                                  table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
                                  ...
                                  th { font-weight: bold; background-color: #fdfbf7; color: #113f48; border-bottom: 2px solid #C9A15A; }
                                  .status-badge { display: inline-block; padding: 2px 8px; border-radius: 9999px; font-size: 10px; font-weight: bold; }
                                  .status-badge.active { background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
                                  .status-badge.inactive { background-color: #fef2f2; color: #b91c1c; border: 1px solid #fecaca; }
                                  .footer { margin-top: 50px; font-size: 11px; color: #888; text-align: center; border-top: 1px solid #eee; padding-top: 20px; }
                                </style>
                              </head>
                              <body>
                                <h1>mediguide4u — Categories Inventory Report</h1>
                                <table>
                                  <thead>
                                    <tr>
                                      <th>Category Name</th>
          ...
          {activeTab === "adsense-readiness" && (() => {
            const thinArticles = posts.filter(p => {
              const text = (p.content || "").replace(/<[^>]*>/g, ""); // strip HTML tags
              const wordCount = text.trim().split(/\s+/).filter(Boolean).length;
              return wordCount < 800;
            }).length;

            const missingMeta = posts.filter(p => !p.summary || p.summary.trim().length === 0).length;

            const missingRefs = posts.filter(p => {
              const raw = (p.content || "").toLowerCase();
              return !raw.includes("references") && !raw.includes("resource guide") && !raw.includes("sources");
            }).length;

            const guidesCount = posts.length;
            const meetsCount = guidesCount >= 30;
            const contentScore = Math.max(0, 100 - (thinArticles * 15) - (missingMeta * 10));
            const seoScore = Math.max(90, Math.round(posts.reduce((acc, p) => acc + (p.seoScore || 90), 0) / (posts.length || 1)));

            const overallScore = Math.round(
              ( (meetsCount ? 30 : (guidesCount / 30) * 30) + 
                (posts.length > 0 ? ((posts.length - thinArticles) / posts.length) * 20 : 0) + 
                (posts.length > 0 ? ((posts.length - missingMeta) / posts.length) * 25 : 0) + 
                25
              )
            );
              ...
              <div className="space-y-6">
                ...
                <div className="bg-white border border-[#C9A15A]/25 p-6 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  ...
                  <button
                    onClick={() => {
                      const auditRows = [
                        { section: "Content Quality", status: contentScore >= 80 ? "Ready" : "Action Required", score: `${contentScore}/100`, details: `${guidesCount} Original Guides Published` },
                        { section: "Technical SEO", status: seoScore >= 80 ? "Ready" : "Action Required", score: `${seoScore}/100`, details: "Schema & Crawling Configured" },
                        { section: "E-E-A-T Compliance", status: "Ready", score: "100/100", details: "Disclaimers & Review Workflows Active" },
                        { section: "Legal Completeness", status: "Ready", score: "100/100", details: "8 Legal Pages Active" }
                      ].map(row => `
                        <tr>
                          <td><strong>${row.section}</strong></td>
                          <td><span class="status-badge ready">${row.status}</span></td>
                          <td>${row.score}</td>
                          <td>${row.details}</td>
                        </tr>
                      `).join("");
                      ...
                      const htmlContent = `
                        <html>
                          <head>
                            <title>mediguide4u - Google AdSense Readiness Audit</title>
                            <style>
                              body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; color: #113f48; padding: 40px; line-height: 1.6; }
                              h1 { color: #113f48; border-bottom: 2px solid #C9A15A; padding-bottom: 10px; margin-bottom: 30px; font-size: 28px; }
                              table { width: 100%; border-collapse: collapse; margin-bottom: 20px; }
                              ...
                              th { font-weight: bold; background-color: #fdfbf7; color: #113f48; border-bottom: 2px solid #C9A15A; }
                              .status-badge { display: inline-block; padding: 2px 8px; border-radius: 9999px; font-size: 10px; font-weight: bold; }
                              .status-badge.ready { background-color: #ecfdf5; color: #047857; border: 1px solid #a7f3d0; }
                              .footer { margin-top: 50px; font-size: 11px; color: #888; text-align: center; border-top: 1px solid #eee; padding-top: 20px; }
                            </style>
                          </head>
                          <body>
                            <h1>mediguide4u — Google AdSense Pre-Flight Compliance Report</h1>
                            <table>
                              <thead>
                                <tr>
                                  <th>Section</th>
                ...
                <div className="bg-gradient-to-br from-[#113F48]/10 via-transparent to-[#C9A15A]/15 border border-[#C9A15A]/25 text-[#113F48] p-6 rounded-3xl shadow-sm flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="space-y-2 text-center md:text-left">
                    <h4 className="text-2xl font-extrabold">Overall AdSense Readiness: {overallScore >= 80 ? "Ready" : "Action Required"}</h4>
                    <p className="text-xs text-stone-600 max-w-lg">
                      {overallScore >= 80 
                        ...
                        : "Your platform requires minor adjustments to meet structural criteria, page limits, or E-E-A-T requirements before applying for Google AdSense."}
                    </p>
                  </div>
                  <div className="w-24 h-24 rounded-full bg-emerald-500/20 border-4 border-emerald-500 flex items-center justify-center flex-shrink-0">
                    <span className="text-2xl font-black text-emerald-800">{overallScore}%</span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: "Content Quality", val: `${contentScore}/100`, status: contentScore >= 85 ? "Optimal" : "Needs Review" },
                    { label: "SEO Health", val: `${seoScore}/100`, status: "Optimal" },
                    ...
                    { label: "Legal Complete", val: "100%", status: "8 Legal Pages" }
                  ].map((stat, i) => (
                    <div key={i} className="bg-white border border-[#C9A15A]/15 p-4.5 rounded-2xl shadow-sm space-y-1">
                      ...
                      <span className={`text-[9px] font-bold ${stat.status === "Needs Review" ? "text-amber-600" : "text-emerald-600"}`}>✓ {stat.status}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="bg-white border border-stone-200 p-5 rounded-2xl shadow-sm space-y-4">
                    ...
                    <ul className="text-xs space-y-2.5 font-semibold text-stone-600">
                      <li className="flex justify-between"><span>Total Published Articles</span> <span className="text-emerald-600">✓ {guidesCount} Guides</span></li>
                      <li className="flex justify-between"><span>Thin Content Pages (&lt; 800 words)</span> <span className={thinArticles > 0 ? "text-amber-600" : "text-emerald-600"}>{thinArticles > 0 ? `⚠️ ${thinArticles} Pages Detected` : "✓ 0 Pages Detected"}</span></li>
                      <li className="flex justify-between"><span>Articles Missing Meta Description</span> <span className={missingMeta > 0 ? "text-amber-600" : "text-emerald-600"}>{missingMeta > 0 ? `⚠️ ${missingMeta} Pages Detected` : "✓ 0 Pages Detected"}</span></li>
                      <li className="flex justify-between"><span>Guides Without References Block</span> <span className={missingRefs > 0 ? "text-amber-600" : "text-emerald-600"}>{missingRefs > 0 ? `⚠️ ${missingRefs} Pages Detected` : "✓ 0 Pages Detected"}</span></li>
                ...
                <div className="bg-[#FDF6EC]/30 border border-[#C9A15A]/25 p-6 rounded-2xl space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 text-xs font-bold text-stone-600">
                    {[
                      meetsCount ? "At least 30 original guides" : `⚠️ ${guidesCount}/30 original guides published`,
                      ...
                      <div key={idx} className="flex items-center gap-2">
                        <span className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] ${check.startsWith("⚠️") ? "bg-amber-50 border border-amber-200 text-amber-600" : "bg-emerald-50 border border-emerald-200 text-emerald-600"}`}>
                          {check.startsWith("⚠️") ? "!" : "✓"}
                        </span>
                        <span>{check.startsWith("⚠️") ? check.substring(2) : check}</span>
                      </div>
                    ))}
                  </div>
                </div>
