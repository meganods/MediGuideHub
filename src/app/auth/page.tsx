...
import { HeartPulse, Lock, Mail, User as UserIcon, ShieldAlert, CheckCircle2, Eye, EyeOff } from "lucide-react";
...
function AuthForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeTab === "signin") {
        ...
        // Show login tab with success message
  ...
  const handleForgotPassword = async () => {
    ...
    setResetMessage("");
    if (!email) {
      setError("Please enter your email address in the field to reset your password.");
      return;
    }
    try {
  ...
  return (
    <div className="w-full max-w-6xl bg-white rounded-3xl overflow-hidden shadow-2xl shadow-stone-200/80 border border-stone-100 flex flex-col lg:flex-row min-h-[720px]">
      ...
      <div className="w-full lg:w-1/2 bg-[#113F48] p-10 sm:p-12 lg:p-16 flex flex-col justify-between text-white relative overflow-hidden select-none">
        ...
        <div className="absolute -bottom-20 -right-20 w-80 h-80 border-2 border-[#C9A15A]/10 rounded-full pointer-events-none animate-pulse" />
      ...
      {/* Right 50% Split-Screen Sliding Form Panel */}
      <div className="w-full lg:w-1/2 relative overflow-hidden bg-white">
        
        {/* Double-width wrapper for sliding effect */}
        <div 
          className="w-[200%] flex flex-row transition-transform duration-500 ease-out h-full"
          style={{ transform: activeTab === "signin" ? "translateX(0%)" : "translateX(-50%)" }}
        >
          {/* 1. Login Panel (left w-1/2 of double container) */}
          <div className="w-1/2 p-8 sm:p-12 lg:p-14 flex flex-col justify-center">
            <div className="space-y-6">
              ...
              <div className="text-center pt-2">
                <p className="text-xs text-stone-500">
                  New here?{" "}
                  <button 
                    onClick={() => { setActiveTab("signup"); setError(""); setResetMessage(""); }}
              ...
              {/* Continue with Google - Login Only */}
              <button
                type="button"
          ...
          {/* 2. Signup Panel (right w-1/2 of double container) */}
          <div className="w-1/2 p-8 sm:p-12 lg:p-14 flex flex-col justify-center bg-white">
            <div className="space-y-6">
              <div>
              ...
              <div className="text-center pt-2">
                <p className="text-xs text-stone-500">
                  ...
                  <button 
                    onClick={() => { setActiveTab("signin"); setError(""); setResetMessage(""); }}
                    className="text-[#C9A15A] hover:underline font-semibold"
                  >
                    Log in
                  </button>
